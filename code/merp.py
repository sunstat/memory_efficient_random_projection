...
class Merp(object):
    ...
    def __init__(self, n, k, rand_type='g', target='col', tensor=False, fastQR=False):
        ...
        # generate omega
        if fastQR and tensor:
            self._omegas, self._omega = self._rm_gen_sp(n, k=k, typ=rand_type, target=target)
        else:
            self._omega = self._rm_gen(n, k=k, typ=rand_type, target=target)

    def regenerate_omega(self):
        if self._fastQR and self._tensor:
            self._omegas, self._omega = self._rm_gen_sp(self._n, k=self._k, typ=self._type, target=self._target)
        else:
            self._omega = self._rm_gen(self._n, k=self._k, typ=self._type, target=self._target)
